...
def get_alias_cluster(schema_meta, reduction_factor, embedding, embedding_dim):
    confusing_cols = defaultdict()
    ver = schema_meta['Version']
    max_score = 0.5
    alias_cluster = defaultdict()

    with open('data/processed/alias_cluster_{0}.json'.format(ver), 'w') as outfile:
        for table in schema_meta['Entities']:
            table_ = defaultdict()
            for idx1 in range(0, len( table['Attributes'])):
                aliases1 = table['Attributes'][idx1]['Aliases']
                col_name1 = table['Attributes'][idx1]['Column']
                natural_words1 = table['Attributes'][idx1]['NaturalWords']
                # Number of words to analyse according to memory availability
                n_words = len(aliases1)
        ...
        json.dump(alias_cluster, outfile)


def get_confusing_columns(schema_meta, embedding):
    # schema_meta structure:
    # account = schema_meta['Entities'][0]['Attributes']
    # opportunity = schema_meta['Entities'][1]['Attributes']
    confusing_cols = defaultdict()
    ver = schema_meta['Version']
    ...
    for table in schema_meta['Entities']:
        ...
        for idx1 in range(0, len(table['Attributes'])):
            col_list = []
            for idx2 in range(0, len(table['Attributes'])):
                col_name1 = table['Attributes'][idx1]['Column']
                col_name2 = table['Attributes'][idx2]['Column']
                
                splitted1 = re.sub('([A-Z][a-z][0-9]+)', r' \1', re.sub('([A-Z]+)', r' \1', col_name1)).split()
                splitted2 = re.sub('([A-Z][a-z][0-9]+)', r' \1', re.sub('([A-Z]+)', r' \1', col_name2)).split()
                print ('calculating ', splitted1, splitted2)
                
                Vector1 = PhraseVector(' '.join(splitted1), embedding)
                Vector2 = PhraseVector(' '.join(splitted2), embedding)
                similarityScore = Vector1.CosineSimilarity(Vector2.vector)
                if similarityScore > max_score:
                    print(col_name1, col_name2, similarityScore)
                    col_list.append(col_name2)
            if col_name1 in col_list:
                col_list.remove(col_name1)
            table_[col_name1] = col_list
        confusing_cols[table['Name']] = table_
